...
function switchTab(tab){
  document.querySelectorAll('.tab').forEach((t,i)=>t.classList.toggle('active', (i===0&&tab==='map')||(i===1&&tab==='rating')));
...
function addToCluster(r){const m=L.marker([r.lat,r.lng],{icon:makeIcon(r.sector)});m.bindPopup(`<b style="font-size:12px">${r.sector}</b><br><span style="font-size:10px;color:#666">${r.addr}</span><br><span style="font-size:11px;margin-top:3px;display:block">${r.official||r.text}</span>`);clusterGroup.addLayer(m);}
...
async function onAddrInput(val){
  ...
  sugTimeout=setTimeout(async()=>{
    try{const r=await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(val+' Uzbekistan')}&format=json&limit=5&accept-language=uz,ru`);const data=await r.json();
    ...
    box.style.display='block';}catch(e){box.style.display='none';}
  },400);
}
